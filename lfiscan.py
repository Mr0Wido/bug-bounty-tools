...
def parser_arguments():
    ...
    parser.add_argument("-p", "--payload", help="Specify the payload file for LFI", required=True)
...
def scan_lfi():
    ...
    try:
        ...
        clean_lfi_command = f"cat {target_list} | grep '=' | qsreplace FUZZ | tee {lfi_fuzzed_urls}"
    ...
    try:
        lfi_command = f"for URL in $(<{lfi_fuzzed_urls}); do (ffuf -u \"${{URL}}\" -w {payload_file}:FUZZ -mc 200 -ac -sa -t 20 -or -od ffuf_lfi_results); done"
    ...
    try:
        ...
        with open('output.txt', "w") as f:
            pass
        file_command =  "grep -o 'ffuf_lfi_results/[^ ]*' lfi_temp_results.txt | xargs -I {} sh -c 'echo \"=============000000============\" >> output.txt; cat {} >> output.txt; echo \"\" >> output.txt'"
    ...
    subprocess.run("rm -rf ffuf_lfi_results", shell=True, check=True)
    subprocess.run("rm -rf lfi_temp_results.txt lfi_fuzzed_urls.txt", shell=True, check=True)
